...
public class HivstMobilizationSessionDetailsActivity extends CoreAncMedicalHistoryActivity {
    private static String baseEntityId;

    private final Flavor flavor = new HivstMobilizationSessionDetailsActivityFlv();

    private ProgressBar progressBar;

    public static void startMe(Activity activity, String baseEntityId) {
        Intent intent = new Intent(activity, HivstMobilizationSessionDetailsActivity.class);
        activity.startActivity(intent);
        HivstMobilizationSessionDetailsActivity.baseEntityId = baseEntityId;
    }

    @Override
    public void initializePresenter() {
        presenter = new BaseAncMedicalHistoryPresenter(new HivstMobilizationSessionDetailsInteractor(), this, baseEntityId);
    }
    ...
    public View renderView(List<Visit> visits) {
        ...
        sbcVisitTitle.setText(R.string.hivst_mobilization_session);
        return view;
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == JsonFormUtils.REQUEST_CODE_GET_JSON && resultCode == RESULT_OK) {
            ...
            try {
                ...
                Timber.e(e, "HivstMobilizationSessionDetailsActivity -- > onActivityResult");
            }
        }
    }
    ...
    private static class HivstMobilizationSessionDetailsActivityFlv extends DefaultAncMedicalHistoryActivityFlv {
        ...
        public void processViewData(List<Visit> visits, Context context) {
            ...
            if (visits.size() > 0) {
                ...
                while (x < visits.size()) {
                    ...
                    String[] clientsReached = {"female_clients_reached", "male_clients_reached"};
                    extractVisitDetails(visits, clientsReached, visitDetails, x, context);

                    String[] condomsIssued = {"female_condoms_issued", "male_condoms_issued",};
                    extractVisitDetails(visits, condomsIssued, visitDetails, x, context);

                    String[] kitsUsedInDemonstration = {"kit_used_in_demonstration", "kit_code"};
                    extractVisitDetails(visits, kitsUsedInDemonstration, visitDetails, x, context);
        ...
        public void startFormForEdit(Integer title_resource, String formName, String baseEntityId, String deletedVisitId, Context context) {
            try {
                ...
                Event event = getEditEvent(baseEntityId, HIVST_MOBILIZATION);

                final List<Obs> observations = event.getObs();
                JSONObject form = getFormWithMetaData(baseEntityId, context, formName, HIVST_MOBILIZATION);
