...
/**
 * ActionHelper for Section A: Clinical Adherence.
 * Extracts clinic attendance and medication adherence responses.
 */
public class NcdClinicalAdherenceActionHelper implements BaseNcdVisitAction.NcdVisitActionHelper {

    private static final String STEP_ONE = "step1";
    private static final String KEY_CLINIC_ATTENDANCE = "clinic_attendance";
    private static final String KEY_MEDICATION_ADHERENCE = "medication_adherence";
    ...
    public String evaluateSubTitle() {
        String clinic = extractResult(KEY_CLINIC_ATTENDANCE);
        String meds = extractResult(KEY_MEDICATION_ADHERENCE);

        boolean hasClinic = StringUtils.isNotBlank(clinic);
        boolean hasMeds = StringUtils.isNotBlank(meds);

        if (!hasClinic && !hasMeds) {
            return context != null
                    ? context.getString(R.string.ncd_followup_clinical_adherence_pending)
                    : "Not yet completed";
        }

        String clinicDisplay = "yes".equalsIgnoreCase(clinic) ? "Attended" : "Missed";
        String medsDisplay = "yes".equalsIgnoreCase(meds) ? "Adherent" : "Non-adherent";

        return String.format(Locale.US, "Clinic: %s | Medication: %s", clinicDisplay, medsDisplay);
    }
    ...
    private boolean isComplete() {
        return StringUtils.isNotBlank(extractResult(KEY_CLINIC_ATTENDANCE))
                && StringUtils.isNotBlank(extractResult(KEY_MEDICATION_ADHERENCE));
    }
    ...
    private String fetchStoredPayload() {
        List<VisitDetail> visitDetails = getVisitDetails(KEY_CLINIC_ATTENDANCE);
